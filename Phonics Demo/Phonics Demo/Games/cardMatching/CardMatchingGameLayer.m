...
+ (CCScene*) scene
{
    ...
    CardMatchingGameLayer *layer = [CardMatchingGameLayer node];
...
//      CGPoint centerScreen = ccp(width*0.5f, height*0.5f);
        ...
        NSArray *words = [self.gameData allKeys];
        
        // preload sound effect
        for (NSString *word in words)
        {
            NSString *wordAudioName = [[self.gameData valueForKey:word] objectForKey:@"audioFile"];
...
#pragma mark -pencil
        ...
        for (NSUInteger i = 0;i<wordsCount;i++)
        {
            ...
            NSString *_frontImgName = [[self.gameData valueForKey:_word] objectForKey:@"imgFile"];
            NSString *_backImgName = @"backCard.png";
            CCSprite *tempFrontImg_1 = [CCSprite spriteWithSpriteFrameName:_frontImgName];
            CCSprite *tempBackImg_1 = [CCSprite spriteWithSpriteFrameName:_backImgName];
            CCSprite *tempFrontImg_2 = [CCSprite spriteWithSpriteFrameName:_frontImgName];
            CCSprite *tempBackImg_2 = [CCSprite spriteWithSpriteFrameName:_backImgName];
            Card *card1 = [Card cardWithWord:_word frontImg:tempFrontImg_1 backImg:tempBackImg_1];
            Card *card2 = [Card cardWithWord:_word frontImg:tempFrontImg_2 backImg:tempBackImg_2];
...
{
    [self removeChild:cardPanel cleanup:YES];
...
{
    CCCallBlock *hide = [CCCallBlock actionWithBlock:^{
        [self hideWithDuration:1.0f];
    }];
    CCDelayTime *delay = [CCDelayTime actionWithDuration:1.05f];
    CCCallBlock *callParent = [CCCallBlock actionWithBlock:^{
...
- (void) onExit
{
    ...
    {
        NSString *wordAudioName = [[self.gameData valueForKey:_card.word] objectForKey:@"audioFile"];
        [[SimpleAudioEngine sharedEngine] unloadEffect:wordAudioName];
    }
    
    [[CCSpriteFrameCache sharedSpriteFrameCache] removeSpriteFramesFromFile:@"cardsSheet.plist"];
}
...
- (BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (_card)
    {
        if (!_card.isShow)
        {
            [_card flipCardFromDirection:FlipFromRight withDuration:0.4];
            NSString *wordAudioName = [[self.gameData valueForKey:_card.word] objectForKey:@"audioFile"];
            [[SimpleAudioEngine sharedEngine] playEffect:wordAudioName];
        }
        ...
        else if (!secondCard)
        {
            ...
            if (secondCard == firstCard)
            {
                ...
                [secondCard hideFromDirection:FlipFromRight withDuration:0.4f];
                firstCard = nil;
                secondCard = nil;
                return NO;
            }
            ...
            CCCallBlock *compare = [CCCallBlock actionWithBlock:^{
                ...
                {
                    //[[SimpleAudioEngine sharedEngine] playEffect:@"buuu.caf"];
                    [firstCard flipCardFromDirection:FlipFromRight withDuration:0.4f];
                    [secondCard flipCardFromDirection:FlipFromRight withDuration:0.4f];
