...
#import "bubble_config.h"

#import "PauseMenu.h"
#import "GameDoneLayer.h"
...
+ (CCScene*) scene
{
    ...
    [scene addChild:[BubbleGameLayer node]];
    return scene;
}
...
- (id) initWithGameData:(NSDictionary *)data
{
    if (self = [super initWithGameData:data])
    {
        ...
        // show title
        CCLabelTTF *title = [CCLabelTTF labelWithString:@"BUBBLE" fontName:@"GillSans-Bold" fontSize:24];
        ...
        CCMenuItemSprite *pauseButton = [CCMenuItemSprite itemWithNormalSprite:pause_button_normal selectedSprite:pause_button_pressed block:^(id sender) {
            [[CCDirector sharedDirector] pushScene:[PauseMenu sceneWithScene:(CCScene*)self]];
        }];
...
//        _scorePopLabelPool = [[CCArray alloc] init];
        ...
        _words = [[self.gameData allKeys] retain];
        ...
        //add wall around, 三面墙,其中下面那面墙在屏幕下方外面 100 点的位置
        {
            ...
            shape = cpSpaceAddShape(_space, cpSegmentShapeNew(staticBody, leftDown, leftUp, radius));
			cpShapeSetFriction(shape, 1.0f);
            cpShapeSetElasticity(shape, 1.0f);
			cpShapeSetLayers(shape, PhysicsEdgeLayers);
            
            shape = cpSpaceAddShape(_space, cpSegmentShapeNew(staticBody, leftDown, rightDown, radius));
			cpShapeSetFriction(shape, 1.0f);
            cpShapeSetElasticity(shape, 1.0f);
			cpShapeSetLayers(shape, PhysicsEdgeLayers);
            
            shape = cpSpaceAddShape(_space, cpSegmentShapeNew(staticBody, rightDown, rightUp, radius));
			cpShapeSetFriction(shape, 1.0f);
            cpShapeSetElasticity(shape, 1.0f);
			cpShapeSetLayers(shape, PhysicsEdgeLayers);
        }
        ...
        for (int i = 'a';i<='z';i++)
        {
            ...
            _bubble.isObjectPerformRotate = NO;
...
- (void) gameDone
{
    ...
    GameDoneLayer *layer = [[[GameDoneLayer alloc] initWithBackScene:(CCScene*)self] autorelease];
    layer.score = _score;
    layer.coin = _coin;
...
{
    CCLOG(@"game over");
    [[CCDirector sharedDirector] pushScene:[GameOverLayer sceneWithBackScene:(CCScene*)self]];
}
